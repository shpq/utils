...
def additional_augmenation_good(image, x, y, mode="torch"):
...
def additional_augmenation_bad(image, x, y, mode="torch"):
    image = np.array(image).astype("uint8")
    compose_list = [
        A.Resize(x, y),
        # A.RandomCrop(120, 120),
...
def additional_augmenation_val(image, x, y, mode="torch"):
    image = np.array(image).astype("uint8")
    compose_list = [A.Resize(x, y),
                    # A.RandomCrop(120, 120),
                    A.Normalize(),
                    # ToTensorV2(),
                    ]
...
def additional_augmenation_strong(image, x, y, mode="torch"):
    image = np.array(image).astype("uint8")
    strong_augs = [
